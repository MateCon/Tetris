...
class GameComponent(DesktopComponent):
    ...
    def rotateLeft(self):
        if self.game.canRotateLeft() and self.game.willLock() and self.hasInfinity and self.infinityCancelCount < 15:
            self.timeSinceLastTick = 0
            self.infinityCancelCount += 1
        self.gameActions.rotateLeft()

    def rotateRight(self):
        if self.game.canRotateRight() and self.game.willLock() and self.hasInfinity and self.infinityCancelCount < 15:
            self.timeSinceLastTick = 0
            self.infinityCancelCount += 1
